...
const FONT_SIZE = 28;
const AUTHOR_FONT_SIZE = Math.round(FONT_SIZE * 0.6);
...
const STICKER_MESH_SCALE = 0.65; // Scale factor to map canvas pixels to 3D units
...
function renderStickerCanvas(sticker: TextStickerEntry): HTMLCanvasElement {
  ...
  const authorHeight = hasAuthor ? AUTHOR_FONT_SIZE + 6 : 0;
  const textWidth = Math.max(...lines.map((l) => ctx.measureText(l).width), 80);
  const canvasW = Math.ceil(textWidth + CANVAS_PADDING * 2);
  const canvasH = Math.ceil(authorHeight + lines.length * LINE_HEIGHT + CANVAS_PADDING * 2);
  ...
  if (hasAuthor) {
    ctx.font = `${AUTHOR_FONT_SIZE}px sans-serif`;
